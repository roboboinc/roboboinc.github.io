...
##
STAGING_BUCKET='s3://staging.robobo.org/'
LIVE_BUCKET='s3://www.robobo.org/'
SITE_DIR='_site/'
...
#red '--> Running Local Image Optimization Script'
#./_imgOptimization.sh
...
s3cmd sync --exclude '*.*' --include '*.css' --mime-type='text/css' --add-header='Content-Type: text/css' --add-header='Cache-Control: max-age=604800' --add-header='Content-Encoding: gzip' $SITE_DIR $BUCKET


yellow '--> Uploading js files'
s3cmd sync --exclude '*.*' --include '*.js' --mime-type='application/javascript' --add-header='Content-Type: application/javascript' --add-header='Cache-Control: max-age=604800' --add-header='Content-Encoding: gzip' $SITE_DIR $BUCKET
...
s3cmd sync --exclude '*.*' --include '*.html' --mime-type='text/html' --add-header='Content-Type: text/html' --add-header='Cache-Control: max-age=7200, must-revalidate' --add-header='Content-Encoding: gzip' $SITE_DIR $BUCKET
